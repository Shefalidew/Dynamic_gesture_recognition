## working code of Inference for Feature-level fusion on BB clipped images
...
model = models.load_model('/home/shefali/Master_Thesis_codes/Working_trained_models/Inference/feature_fusion_BB_trained.h5',compile=False)
# Load YOLOv5 model
...
def images_from_frame(color_frame,depth_frame):
    ...
    return depth_image,color_image,combined_data

def reduce_size(image):
    image_reduced = cv2.resize(cropped_image, (96,96))
    depth_image_reduced = image_reduced[:,:,-1]
    color_image_reduced = image_reduced[:,:,0:3]
    return depth_image_reduced,color_image_reduced
...
if __name__ == '__main__':
    ...
    while TRUE:
        ...
        while i < frames_capture:
            ...
            depth_image,color_image, combined_data = images_from_frame(color_frame,depth_frame)
            eBB =ExtractBoudingBoxRegion(combined_data)
            cropped_image = eBB.get_cropped_image()
            depth_image_reduced,color_image_reduced = reduce_size(cropped_image)
            depth_data.append(depth_image_reduced)
            color_data.append(color_image_reduced)
            i=i+1
        ...
        depth_img_data = np.array(depth_data) 
        depth_img_data = np.expand_dims(depth_img_data, axis=-1)
        depth_img_data = scaling(depth_img_data)
        depth_img_data = depth_img_data.reshape((1,depth_img_data.shape[0], depth_img_data.shape[1],depth_img_data.shape[2],depth_img_data.shape[3]))
        
        color_img_data = np.array(color_data) 
        color_img_data = scaling(color_img_data)
        color_img_data = color_img_data.reshape((1,color_img_data.shape[0], color_img_data.shape[1],color_img_data.shape[2],color_img_data.shape[3]))
        
        predict = model.predict([color_img_data,depth_img_data])
